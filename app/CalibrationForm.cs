...
namespace MainApp
{
    ...
    public partial class CalibrationForm : Form
    {
        ...
        public CalibrationForm(string calibrationFile = "")
        {
            ...
            {
                this.CalibrationFile = Path.GetFullPath(calibrationFile);
                txtCalibFile.Text = this.CalibrationFile;
            }
            LoadCalibration();
        }
        ...
        private void LoadCalibration()
        {
            ...
            // Set up the controls
            SetGraph();
            nudSlope.Value = (decimal)this.Slope;
            nudIntercept.Value = (decimal)this.Intercept;
            dgCalibration.Rows.Clear();
            for (int i = 0; i < _sensorTemps.Count; i++)
            {
                dgCalibration.Rows.Add(new string[] { _sensorTemps[i].ToString("#.##"), _correctTemps[i].ToString("#.##") });
            }
        }

        private void NudSlope_ValueChanged(object sender, EventArgs e)
        {
            this.Slope = (double)nudSlope.Value;
            SetGraph();
        }

        private void NudIntercept_ValueChanged(object sender, EventArgs e)
        {
            this.Intercept = (double)nudIntercept.Value;
            SetGraph();
        }
